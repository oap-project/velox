...
void applyCastKernel(
  ...
  if constexpr (CppToType<To>::typeKind == TypeKind::VARCHAR) {
    ...
    if constexpr (
      ...
      output = util::Converter<
          CppToType<To>::typeKind,
          void,
          Truncate,
          AllowDecimal,
          TrimWhitespace>::cast(input->valueAt(row), nullOutput, input->type());
      ...
      output = util::Converter<
          ...
          Truncate,
          AllowDecimal,
          TrimWhitespace>::cast(input->valueAt(row), nullOutput);
    }
