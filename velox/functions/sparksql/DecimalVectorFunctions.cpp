 ...
 public:
  ...
  void apply(
    ...
    if constexpr (std::is_same_v<T, int64_t>) {
      int128_t bound = velox::DecimalUtil::kPowersOfTen[precision_];
...
class DecimalRoundFunction : public exec::VectorFunction {
 ...
 private:
  ...
  inline void applyRound(
      ...
      int32_t scale) const {
    if (scale >= 0) {
      auto rescaledValue = velox::DecimalUtil::rescaleWithRoundUp<A, R>(
          a, aPrecision, aScale, rPrecision, rScale);
      VELOX_DCHECK(rescaledValue.has_value());
      r = rescaledValue.value();
    } else {
      auto reScaleFactor = velox::DecimalUtil::kPowersOfTen[aScale - scale];
      velox::DecimalUtil::divideWithRoundUp<R, A, int128_t>(
          r, a, reScaleFactor, false, 0, 0);
      r = r * velox::DecimalUtil::kPowersOfTen[-scale];
    }
  }
};
