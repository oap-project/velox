...
class ComparisonFunction final : public exec::VectorFunction {
  ...
  void apply(
    ...
    auto resultRef =
        result->asUnchecked<FlatVector<bool>>()->mutableRawValues();
    const Cmp cmp;
    if (!args[0]->mayHaveNulls() && !args[1]->mayHaveNulls()) {
      // When there is no nulls, it reduces to normal compare function
      rows.applyToSelected([&](vector_size_t i) {
        resultRef[i] = cmp(decoded0->valueAt<T>(i), decoded1->valueAt<T>(i));
      });
    } else {
      rows.applyToSelected([&](vector_size_t i) { result->setNull(i, true); });
    }
  }
};
