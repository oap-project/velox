 ...
 public:
  ...
  void apply(
    ...
    if (resultType_->isInteger()) {
      applyBlock<int32_t>(peerGroupStarts, resultOffset, result);
    } else {
      applyBlock<int64_t>(peerGroupStarts, resultOffset, result);
    }
  }
...
void registerRowNumber(
    const std::string& name,
    const TypeKind& offsetTypeKind) {
  std::vector<exec::FunctionSignaturePtr> signatures{
      exec::FunctionSignatureBuilder()
          .returnType(mapTypeKindToName(offsetTypeKind))
          .build(),
  };
